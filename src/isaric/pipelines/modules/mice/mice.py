...
class MICEImputer:
    ...
    def fit(self, df, interest_vars=None, show_results=True):
        ...
        ----------
        df : pd.DataFrame
            Input dataset. May contain missing values and both numerical
            and categorical (object / category dtype) columns.
        interest_vars : list of str or None, default=None
            Columns to include in the statistical analysis table.
            Columns without missing values are used as predictors only.
            If None, every column in df is used.
        show_results : bool, default=True
            Whether to display the statistical analysis table after fitting.
    ...
    def get_results(self):
        """
        Return the results produced by fit().

        Returns
        -------
        imputed_datasets : list of pd.DataFrame
            The n individually imputed datasets (also saved to
            'imputed_datasets.zip').
        pooled_df : pd.DataFrame
            Pooled dataset (element-wise mean across imputations).
        stats_df : pd.DataFrame
            Comparative statistical analysis table.
    ...
    # ==================================================================
    # Private: input validation & seed generation
    # ==================================================================
    ...
    def _validate_inputs(self, df, interest_vars):
        if interest_vars is not None:
            ...
            if missing:
                raise ValueError(
                    f"Interest variables not found in dataframe columns: {missing}"
                )
    ...
    def _encode_categoricals(self, df):
        """One-hot encode all object/category columns. Returns (encoded_df, cat_levels)."""
        cat_levels = {}
        categorical_cols = df.select_dtypes(include=['object', 'category']).columns.tolist()

        for col in categorical_cols:
            dummies = pd.get_dummies(
                df[col], prefix=col, prefix_sep=self.prefix_sep, dummy_na=False
            )
            cat_levels[col] = list(dummies.columns)
            df = pd.concat([df.drop(columns=[col]), dummies], axis=1)

        # Also detect pre-existing dummy blocks in the (possibly already-encoded) df:
    ...
    def _coerce_to_numeric(self, df):
        """Coerce any remaining non-numeric columns to numeric (errors → NaN)."""
    ...
    def _reconstruct_from_expanded(self, expanded_df, original_df):
        """
        Reconstruct categorical columns from one-hot dummy columns in
        expanded_df and reorder to match original_df column order.
        """
        ...
        for var, dummy_cols in self._cat_levels.items():
            ...
            if len(present) == 1:
                ...
                if self.force_binary:
                    reconstructed[var] = (
                        expanded_df[col_name] >= this_cutoff
                    ).astype('Int64')
                else:
                ...
                for idx in range(expanded_df.shape[0]):
                    ...
                    winner_idx, _ = self._reconstruct_categorical_from_dummies(
                        row_vals, present, this_cutoff, var
                    )
                    selected.append(
                        np.nan if winner_idx is None else categories[winner_idx]
                    )
                reconstructed[var] = pd.Series(
                    selected, index=expanded_df.index, dtype='object'
                )
                reconstructed = reconstructed.drop(columns=present)
    ...
    def _run_imputations(self, original_df):
        """Run n independent IterativeImputer calls. Returns (list_of_dfs, stacked_arrays)."""
        imputed_datasets = []
        stacked_arrays = []
        ...
        for seed in self._seeds:
            ...
            expanded_df = pd.DataFrame(
                arr,
                index=self._df_for_impute.index,
                columns=self._df_for_impute.columns,
            )
    ...
    def _build_pooled(self, original_df):
        """Pool n imputed arrays (element-wise mean) and reconstruct categoricals."""
    ...
    def _build_stats(self, df, interest_vars):
        """Build comparative stats table for all interest_vars that have missings."""
        stats = []
    ...
    def _calculate_p_value_categorical(series_before, series_after):
        ...
        all_categories = sorted(list(
            set(counts_before.index).union(set(counts_after.index))
        ))
    ...
    def _get_categorical_blocks(self, df):
        """
        Detect dummy blocks (columns containing prefix_sep) and standalone
        binary columns (0/1 only). Returns (blocks_dict, single_binary_list).
        """
        ...
        for col in df.columns:
            if self.prefix_sep in col:
                ...
                if len(unique_nonnull) > 0 and all(
                    x in {0, 1, 0.0, 1.0} for x in unique_nonnull
                ):
    ...
    def _reconstruct_categorical_from_dummies(self, df_row_values, dummy_cols, cutoff, var_name):
        """
        Given a 1-D array of imputed dummy values for one row, determine the
        winning class index according to cutoff, tie_strategy, and tie_force.
        Returns (winner_idx, vals) where winner_idx may be None.
        """
        ...
        if num_ge == 1:
            ...
            # Tie among values that are >= cutoff
    ...
    def _resolve_forced_class(self, dummy_cols, var_name, vals):
        ...
        if isinstance(self.tie_force, dict):
            if var_name not in self.tie_force:   # <-- missing guard
                return None, vals

...
The `RAPID_LinearRegression` class inherits core functionality from `RAPID_BaseRegression`, which provides:
...
* **summary_df** (*pd.DataFrame*): Results summary with coefficients and CIs
...
| `_modeling` | Fits a statsmodels GLM with Gaussian family (equivalent to OLS). |
| `_model_evaluation` | Executes assumption tests, performance metrics, and cross-validation. |
| `_test_assumptions` | Runs Durbin-Watson, Shapiro-Wilk, VIF, and Cook's Distance tests. |
| `_test_performance_metrics` | Calculates MSE, RMSE, MAE, R², and Adjusted R². |
| `_test_cross_validation` | Performs k-fold cross-validation using sklearn. |
| `_build_result_summary_df` | Constructs summary DataFrame with coefficients and confidence intervals. |
